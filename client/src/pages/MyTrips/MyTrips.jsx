...
export default function MyTrips() {
  ...
  const [trips, setTrips] = useState(contextTrips);
  ...
  useEffect(() => {
    const fetchTrips = async () => {
      try {
        ...
        const apiTrips = Array.isArray(response.response) ? response.response : [];
        // Combine context trips with API trips without duplicates
        const combined = [...contextTrips];
        apiTrips.forEach(at => {
          if (!combined.some(ct => String(ct.id) === String(at.id))) {
            combined.push(at);
          }
        });
        setTrips(combined);
        ...
        // Fallback to local context trips
